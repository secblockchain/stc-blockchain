...
func (api *ExternalSigner) SignData(account accounts.Account, mimeType string, data []byte) ([]byte, error) {
	...
	// If V is on 27/28-form, convert to 0/1 for Clique and Stcons
	if (mimeType == accounts.MimetypeClique || mimeType == accounts.MimetypeStcons) && (res[64] == 27 || res[64] == 28) {
		res[64] -= 27 // Transform V from 27/28 to 0/1 for Clique and Stcons use
	}
	return res, nil
}
