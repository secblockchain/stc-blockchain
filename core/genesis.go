...
func getGenesisState(db ethdb.Database, blockhash common.Hash) (alloc types.GenesisAlloc, err error) {
	...
	case params.STCGenesisHash:
		genesis = DefaultSTCGenesisBlock()
	case params.StcTestnetGenesisHash:
		genesis = DefaultStcTestnetGenesisBlock()
	}
	if genesis != nil {
...
// Typically, these modifications involve hardforks that are not enabled on the STC mainnet, intended for testing purposes.
...
func SetupGenesisBlockWithOverride(db ethdb.Database, triedb *triedb.Database, genesis *Genesis, overrides *ChainOverrides, tracer *tracing.Hooks) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
	...
	if (ghash == common.Hash{}) {
		if genesis == nil {
			log.Info("Writing default STC mainnet genesis block")
			genesis = DefaultSTCGenesisBlock()
		} else {
...
func LoadChainConfig(db ethdb.Database, genesis *Genesis) (cfg *params.ChainConfig, ghash common.Hash, err error) {
	...
	return params.STCChainConfig, params.STCGenesisHash, nil
}
...
func (g *Genesis) toBlockWithRoot(root common.Hash) *types.Block {
	...
	if g.Config != nil && g.Config.IsLondon(common.Big0) {
		if g.BaseFee != nil {
			...
			if g.Config.IsInSTC() {
				head.BaseFee = new(big.Int).SetUint64(params.InitialBaseFeeForSTC)
			} else {
	...
	if conf := g.Config; conf != nil {
		...
		if conf.IsNotInSTC() && conf.IsShanghai(num, g.Timestamp) {
		...
		if conf.IsCancun(num, g.Timestamp) {
			...
			// by definition.
			if conf.IsNotInSTC() || conf.IsBohr(num, g.Timestamp) {
...
// DefaultSTCGenesisBlock returns the STC mainnet genesis block.
func DefaultSTCGenesisBlock() *Genesis {
	alloc := decodePrealloc(stcMainnetAllocData)
	return &Genesis{
...
// DefaultStcTestnetGenesisBlock returns the STC testnet genesis block.
func DefaultStcTestnetGenesisBlock() *Genesis {
	alloc := decodePrealloc(stcTestnetAllocData)
	return &Genesis{
		Config:     params.StcTestnetConfig,
		Nonce:      0,
