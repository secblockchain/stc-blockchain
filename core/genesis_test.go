...
func testSetupGenesis(t *testing.T, scheme string) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantHash:   params.STCGenesisHash,
			wantConfig: params.STCChainConfig,
		},
		{
			name: "mainnet block in DB, genesis == nil",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
				DefaultSTCGenesisBlock().MustCommit(db, triedb.NewDatabase(db, newDbConfig(scheme)))
				return SetupGenesisBlock(db, triedb.NewDatabase(db, newDbConfig(scheme)), nil)
			},
			wantHash:   params.STCGenesisHash,
			wantConfig: params.STCChainConfig,
		...
		{
			name: "stc testnet block in DB, genesis == nil",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
				DefaultStcTestnetGenesisBlock().MustCommit(db, triedb.NewDatabase(db, newDbConfig(scheme)))
				return SetupGenesisBlock(db, triedb.NewDatabase(db, newDbConfig(scheme)), nil)
			},
			wantHash:   params.StcTestnetGenesisHash,
			wantConfig: params.StcTestnetConfig,
		},
		{
			name: "stc testnet block in DB, genesis == stc testnet",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
				DefaultStcTestnetGenesisBlock().MustCommit(db, triedb.NewDatabase(db, newDbConfig(scheme)))
				return SetupGenesisBlock(db, triedb.NewDatabase(db, newDbConfig(scheme)), DefaultStcTestnetGenesisBlock())
			},
			wantHash:   params.StcTestnetGenesisHash,
			wantConfig: params.StcTestnetConfig,
		},
		...
		{
			name: "custom block in DB, genesis == stc testnet",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
				tdb := triedb.NewDatabase(db, newDbConfig(scheme))
				customg.Commit(db, tdb, nil)
				return SetupGenesisBlock(db, tdb, DefaultStcTestnetGenesisBlock())
			},
			wantErr: &GenesisMismatchError{Stored: customghash, New: params.StcTestnetGenesisHash},
		},
		{
