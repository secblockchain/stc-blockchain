...
func postExecution(ctx context.Context, config *params.ChainConfig, block *types.Block, allLogs []*types.Log, evm *vm.EVM) (requests [][]byte, err error) {
	...
	if config.IsPrague(block.Number(), block.Time()) && config.IsNotInSTC() {
		requests = [][]byte{}
		// EIP-6110
...
func ProcessBeaconBlockRoot(beaconRoot common.Hash, evm *vm.EVM) {
	// Return immediately if beaconRoot equals the zero hash when using the Stcons engine.
	if beaconRoot == (common.Hash{}) {
		if chainConfig := evm.ChainConfig(); chainConfig != nil && chainConfig.IsInSTC() {
			return
		}
	}
