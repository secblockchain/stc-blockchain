...
// TestValidateBlobTx_STCOnlyV0PostOsaka pins the STC txpool admission policy:
// on STC chains (Stcons configured) only version-0 blob sidecars are accepted
// after Osaka is active, since cell proofs (version 1) are not supported.
func TestValidateBlobTx_STCOnlyV0PostOsaka(t *testing.T) {
	key, _ := crypto.GenerateKey()
	v0 := validBlobSidecar(1)

	for _, config := range []*params.ChainConfig{params.STCChainConfig, params.StcTestnetConfig} {
		if config.Stcons == nil || config.OsakaTime == nil {
			t.Fatal("config must have Stcons and OsakaTime set")
		}
		head := &types.Header{
