...
func InspectDatabase(db ethdb.Database, keyPrefix, keyStart []byte) error {
	...
	inspectRange := func(ctx context.Context, r byte) error {
		...
		for it.Next() {
			...
			case bytes.HasPrefix(key, StconsSnapshotPrefix) && len(key) == 7+common.HashLength:
				stconsSnaps.add(size)
	...
	stats := [][]string{
		...
		{"Key-Value store", "Stcons snapshots", stconsSnaps.sizeString(), stconsSnaps.countString()},
	}
