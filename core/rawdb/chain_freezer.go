...
const (
	...
	// For STC, freezing 30,000 blocks at once will seriously affect performance near the chain head.
...
func (f *chainFreezer) freeze(db ethdb.KeyValueStore, continueFreeze bool) {
	...
	for {
		...
		// STC does not use the finalized block as the freeze indicator, for two reasons:
		...
		// TODO(galaio): Temporarily comment that the current STC is suitable for small-volume writes,
