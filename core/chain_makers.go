...
func (b *BlockGen) AddUncle(h *types.Header) {
	...
	if b.cm.config.IsLondon(h.Number) {
		...
		if b.cm.config.IsNotInSTC() && !b.cm.config.IsLondon(parent.Number) {
...
func (b *BlockGen) collectRequests(readonly bool) (requests [][]byte) {
	...
	if b.cm.config.IsPrague(b.header.Number, b.header.Time) && b.cm.config.IsNotInSTC() {
		requests = [][]byte{}
		// EIP-6110 deposits
...
func (cm *chainMaker) makeHeader(parent *types.Block, state *state.StateDB, engine consensus.Engine) *types.Header {
	...
	if cm.config.IsLondon(header.Number) {
		...
		if cm.config.IsNotInSTC() && !cm.config.IsLondon(parent.Number()) {
