...
// ProtocolName is the official short name of the `stc` protocol used during
// devp2p capability negotiation.
const ProtocolName = "stc"

// ProtocolVersions are the supported versions of the `stc` protocol (first
// is primary).
var ProtocolVersions = []uint{Stc1, Stc2}

// protocolLengths are the number of implemented message corresponding to
// different protocol versions.
var protocolLengths = map[uint]uint64{Stc1: 2, Stc2: 4}

// maxMessageSize is the maximum cap on the size of a protocol message.
const maxMessageSize = 10 * 1024 * 1024

const (
	StcCapMsg           = 0x00 // stc capability msg used upon handshake
	VotesMsg            = 0x01
...
// Packet represents a p2p message in the `stc` protocol.
type Packet interface {
	Name() string // Name returns a string corresponding to the message type.
	Kind() byte   // Kind returns the message type.
}

// StcCapPacket is the network packet for stc capability message.
type StcCapPacket struct {
...
func (*StcCapPacket) Name() string { return "StcCap" }
func (*StcCapPacket) Kind() byte   { return StcCapMsg }
