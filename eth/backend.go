...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	if posa, ok := eth.engine.(consensus.PoSA); ok {
		...
		if stconsEngine, ok := eth.engine.(*stcons.Stcons); ok {
			if !config.Miner.DisableVoteAttestation {
				// if there is no VotePool in Stcons Engine, the miner can't get votes for assembling
				stconsEngine.VotePool = votePool
			}
		} else {
			return nil, errors.New("Engine is not Stcons type")
		}
...
func (s *Ethereum) APIs() []rpc.API {
	...
	if p, ok := s.engine.(*stcons.Stcons); ok {
...
func (s *Ethereum) waitForSyncAndMaxwell(stcons *stcons.Stcons) {
...
func (s *Ethereum) updateNodeID(stcons *stcons.Stcons) error {
	...
	registeredIDs, err := stcons.GetNodeIDs()
	if err != nil {
	...
	if err := s.handleRemovals(stcons, nonce, registeredSet); err != nil {
		return err
	}
	nonce++

	// Handle additions
	return s.handleAdditions(stcons, nonce, registeredSet)
}

func (s *Ethereum) handleRemovals(stcons *stcons.Stcons, nonce uint64, registeredSet map[enode.ID]struct{}) error {
	...
	if len(s.config.EVNNodeIDsToRemove) == 1 {
		...
		if s.config.EVNNodeIDsToRemove[0] == zeroID {
			trx, err := stcons.RemoveNodeIDs([]enode.ID{}, nonce)
			if err != nil {
	...
	trx, err := stcons.RemoveNodeIDs(nodeIDsToRemove, nonce)
	if err != nil {
		return fmt.Errorf("failed to create node ID removal transaction: %v", err)
	}
...
func (s *Ethereum) handleAdditions(stcons *stcons.Stcons, nonce uint64, registeredSet map[enode.ID]struct{}) error {
	...
	trx, err := stcons.AddNodeIDs(nodeIDsToAdd, nonce)
	if err != nil {
...
func (s *Ethereum) StartMining() error {
	...
	if !s.IsMining() {
		...
		if stconsEngine, ok := s.engine.(*stcons.Stcons); ok {
			wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
			if wallet == nil || err != nil {
				log.Error("Etherbase account unavailable locally", "err", err)
				return fmt.Errorf("signer missing: %v", err)
			}
			stconsEngine.Authorize(eb, wallet.SignData, wallet.SignTx)

			// Start a goroutine to handle node ID registration after sync
			go func() {
				s.waitForSyncAndMaxwell(stconsEngine)
			}()
		}
...
func (s *Ethereum) Protocols() []p2p.Protocol {
	...
	protos = append(protos, stc.MakeProtocols((*stcHandler)(s.handler))...)
...
func (s *Ethereum) setupDiscovery() error {
	...
	// Add stc nodes from DNS.
	if len(s.config.StcDiscoveryURLs) > 0 {
		iter, err := dnsclient.NewIterator(s.config.StcDiscoveryURLs...)
		if err != nil {
			return err
		}
		s.discmix.AddSource(iter)
