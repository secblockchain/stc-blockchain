...
type testStcHandler struct {
	voteBroadcasts event.Feed
}

func (h *testStcHandler) Chain() *core.BlockChain { panic("no backing chain") }
func (h *testStcHandler) RunPeer(peer *stc.Peer, handler stc.Handler) error {
	panic("not used in tests")
}
func (h *testStcHandler) PeerInfo(enode.ID) interface{} { panic("not used in tests") }
func (h *testStcHandler) Handle(peer *stc.Peer, packet stc.Packet) error {
	...
	default:
		panic(fmt.Sprintf("unexpected stc packet type in tests: %T", packet))
	}
}
...
func testSendVotes(t *testing.T, protocol uint) {
	...
	p2pStcSrc, p2pStcSink := p2p.MsgPipe()
	defer p2pStcSrc.Close()
	defer p2pStcSink.Close()

	localStc := stc.NewPeer(stc.Stc1, p2p.NewPeerWithProtocols(enode.ID{1}, protos, "", caps), p2pStcSrc)
	remoteStc := stc.NewPeer(stc.Stc1, p2p.NewPeerWithProtocols(enode.ID{3}, protos, "", caps), p2pStcSink)
	defer localStc.Close()
	defer remoteStc.Close()

	go func(p *stc.Peer) {
		(*stcHandler)(handler.handler).RunPeer(p, func(peer *stc.Peer) error {
			return stc.Handle((*stcHandler)(handler.handler), peer)
		})
	}(localStc)
...
func testRecvVotes(t *testing.T, protocol uint) {
	...
	remoteStc.AsyncSendVotes([]*types.VoteEnvelope{&vote})
