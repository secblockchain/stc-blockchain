...
function init_genesis_data() {
     ...
     sed -i -e "s/{{NetworkId}}/${STC_CHAIN_ID}/g" ${workspace}/storage/${node_id}/config.toml
     if [ "${node_id}" == "stc-rpc" ]; then
...
for((i=1;i<=${NUMS_OF_VALIDATOR};i++)); do
     init_validator "stc-validator${i}"
done
...
init_genesis_data stc-rpc stc-rpc

for((i=1;i<=${NUMS_OF_VALIDATOR};i++)); do
     init_genesis_data validator "stc-validator${i}"
done

#Step 4, prepare bls wallet, used by fast finality vote
prepareBLSWallet stc-rpc

for((i=1;i<=${NUMS_OF_VALIDATOR};i++)); do
     prepareBLSWallet "stc-validator${i}"
done
