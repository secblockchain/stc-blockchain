...
func (w *worker) newWorkLoop(recommit time.Duration) {
	...
	for {
		...
		case head := <-w.chainHeadCh:
			...
			timestamp = time.Now().Unix()
			if p, ok := w.engine.(*stcons.Stcons); ok {
		...
		case <-timer.C:
			...
			if w.isRunning() && ((w.chainConfig.Clique != nil &&
				w.chainConfig.Clique.Period > 0) || (w.chainConfig.IsInSTC())) {
...
func (w *worker) makeEnv(parent *types.Header, header *types.Header, coinbase common.Address,
	...
	// Stcons reserves gas for the system txs it applies in FinalizeAndAssemble,
...
func (w *worker) prepareWork(genParams *generateParams, witness bool) (*environment, error) {
	...
	if w.chainConfig.IsLondon(header.Number) {
		...
		if w.chainConfig.IsNotInSTC() && !w.chainConfig.IsLondon(parent.Number) {
...
func (w *worker) generateWork(genParam *generateParams, witness bool) *newPayloadResult {
	...
	var requests [][]byte
	if w.chainConfig.IsPrague(work.header.Number, work.header.Time) && w.chainConfig.IsNotInSTC() {
		requests = [][]byte{}
		// EIP-6110 deposits
...
func (w *worker) delay(header *types.Header, leftOver *time.Duration) *time.Duration {
	...
	if p, ok := w.engine.(*stcons.Stcons); ok && p.IsLastBlockInTurn(w.chain, header) {
	...
	for {
		...
		if delay == nil {
			...
			if !w.inTurn() && len(workList) == 1 {
				if stconsEngine, ok := w.engine.(*stcons.Stcons); ok {
					...
					if blockInterval, err := stconsEngine.BlockInterval(w.chain, w.chain.CurrentBlock()); err == nil {
...
func (w *worker) tryWaitProposalDoneWhenStopping() {
	stconsEngine, ok := w.engine.(*stcons.Stcons)
	// if the consensus is not stcons, just skip waiting
	if !ok {
		return
	}
	...
	startBlock, endBlock, err := stconsEngine.NextProposalBlock(w.chain, currentHeader, w.coinbase)
	if err != nil {
	...
	blockInterval, err := stconsEngine.BlockInterval(w.chain, currentHeader)
	if err != nil {
