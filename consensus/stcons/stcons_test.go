...
func TestStcons_applyTransactionTracing(t *testing.T) {
	frdir := t.TempDir()
	...
	engine := New(params.StconsTestChainConfig, db, nil, genesisBlock.Hash())
	...
	cx := chainContext{ChainHeaderReader: chain, stcons: engine}
...
func TestStcons_applyTransactionModes(t *testing.T) {
	frdir := t.TempDir()
	db, err := rawdb.Open(rawdb.NewMemoryDatabase(), rawdb.OpenOptions{Ancient: frdir})
	if err != nil {
...
// TestStcons_applyTransactionReceiptCumulativeGas verifies that a system
...
func TestStcons_applyTransactionReceiptCumulativeGas(t *testing.T) {
	frdir := t.TempDir()
	db, err := rawdb.Open(rawdb.NewMemoryDatabase(), rawdb.OpenOptions{Ancient: frdir})
	if err != nil {
		t.Fatalf("failed to create database with ancient backend: %v", err)
...
func TestStconsFinalizeAndAssembleBidBlockRewardsHeaderCoinbase(t *testing.T) {
	frdir := t.TempDir()
	db, err := rawdb.Open(rawdb.NewMemoryDatabase(), rawdb.OpenOptions{Ancient: frdir})
	if err != nil {
		t.Fatalf("failed to create database with ancient backend: %v", err)
...
func TestStconsPrepareForBidBlock(t *testing.T) {
	frdir := t.TempDir()
	db, err := rawdb.Open(rawdb.NewMemoryDatabase(), rawdb.OpenOptions{Ancient: frdir})
	if err != nil {
		t.Fatalf("failed to create database with ancient backend: %v", err)
	...
	// prepared back-to-back land on the same nextBlockMilliTime output.
...
type mockStcons struct {
	consensus.Engine
}

func (c *mockStcons) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
}

func (c *mockStcons) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	return nil
}

func (c *mockStcons) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header) error {
	return nil
}

func (c *mockStcons) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header) (chan<- struct{}, <-chan error) {
...
func (c *mockStcons) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state vm.StateDB, _ *[]*types.Transaction, uncles []*types.Header, withdrawals []*types.Withdrawal,
	_ *[]*types.Receipt, _ *[]*types.Transaction, _ *uint64, tracer *tracing.Hooks) (err error) {
	return
}

func (c *mockStcons) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, body *types.Body, receipts []*types.Receipt, tracer *tracing.Hooks) (*types.Block, []*types.Receipt, error) {
	// Finalize block
...
func (c *mockStcons) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
	return big.NewInt(1)
}
