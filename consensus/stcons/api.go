...
func (api *API) GetSnapshot(number *rpc.BlockNumber) (*Snapshot, error) {
	...
	return api.stcons.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
}
...
func (api *API) GetValidators(number *rpc.BlockNumber) ([]common.Address, error) {
	...
	snap, err := api.stcons.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
	if err != nil {
		return nil, err
	}
...
func (api *API) getHeader(number *rpc.BlockNumber) (header *types.Header) {
	...
	if number == nil || *number == rpc.LatestBlockNumber {
		...
		justifiedNumber, _, err := api.stcons.GetJustifiedNumberAndHash(api.chain, []*types.Header{currentHeader})
		if err != nil {
			return nil
		}
		header = api.chain.GetHeaderByNumber(justifiedNumber)
	} else if *number == rpc.FinalizedBlockNumber {
		header = api.stcons.GetFinalizedHeader(api.chain, currentHeader)
	} else if *number == rpc.PendingBlockNumber {
		return nil // no pending blocks on stc
