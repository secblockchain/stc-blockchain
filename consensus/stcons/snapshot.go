...
type Snapshot struct {
	config   *params.StconsConfig // Consensus engine parameters to fine tune behavior
...
func newSnapshot(
	...
	snap := &Snapshot{
		...
		BlockInterval:    blockIntervalMs(config),
...
func loadSnapshot(config *params.StconsConfig, sigCache *lru.Cache[common.Hash, common.Address], db ethdb.Database, hash common.Hash, ethAPI *ethapi.BlockChainAPI) (*Snapshot, error) {
	blob, err := db.Get(append([]byte("stcons-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(Snapshot)
	...
	if snap.BlockInterval == 0 { // no BlockInterval field in old snapshots
		snap.BlockInterval = blockIntervalMs(config)
	}
...
func (s *Snapshot) store(db ethdb.Database) error {
	...
	return db.Put(append([]byte("stcons-"), s.Hash[:]...), blob)
}
