...
const (
	...
	validatorNumberSize  = 1 // Extra prefix byte reserved for validator count

	wiggleTime         uint64 = 1000 // milliseconds, Random delay (per signer) to allow concurrent signers
	initialBackOffTime uint64 = 2000 // milliseconds, backoff for the second validator permitted to produce blocks
...
var (
	...
	verifyVoteAttestationErrorCounter = metrics.NewRegisteredCounter("stcons/verifyVoteAttestation/error", nil)
	validVotesfromSelfCounter         = metrics.NewRegisteredCounter("stcons/VerifyVote/self", nil)
	doubleSignCounter                 = metrics.NewRegisteredCounter("stcons/doublesign", nil)
	intentionalDelayMiningCounter     = metrics.NewRegisteredCounter("stcons/intentionalDelayMining", nil)
...
// StconsRLP returns the rlp bytes which needs to be signed for the stcons
...
func StconsRLP(header *types.Header, chainId *big.Int) []byte {
	b := new(bytes.Buffer)
	types.EncodeSigHeader(b, header, chainId)
	return b.Bytes()
}

// Stcons is the consensus engine of Smart Technology Chain
type Stcons struct {
	chainConfig *params.ChainConfig  // Chain config
	config      *params.StconsConfig // Consensus engine configuration parameters for stcons consensus
...
func New(
	...
	// get stcons config
	stconsConfig := chainConfig.Stcons
	log.Info("Stcons", "chainConfig", chainConfig)
...
func (p *Stcons) IsSystemTransaction(tx *types.Transaction, header *types.Header) (bool, error) {
	if tx.To() == nil || !isToSystemContract(*tx.To()) {
		return false, nil
	}
	if tx.EffectiveGasPriceForSTC().Sign() != 0 {
		return false, nil
	}
...
func (p *Stcons) IsSystemContract(to *common.Address) bool {
	if to == nil {
		return false
	}
...
func (p *Stcons) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
}

// ConsensusAddress returns the consensus address of the validator
func (p *Stcons) ConsensusAddress() common.Address {
	return p.val
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (p *Stcons) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (p *Stcons) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header) (chan<- struct{}, <-chan error) {
...
// Epoch extra format:
// |---Extra Vanity---|---Count, Validator Bytes (addr+BLS), Turn Length---|---Vote Attestation (or Empty)---|---Extra Seal---|
func getValidatorBytesFromHeader(header *types.Header, chainConfig *params.ChainConfig, epochLength uint64) []byte {
	...
	extraMinLen := end + extraSeal + turnLengthSize
...
func getVoteAttestationFromHeader(header *types.Header, chainConfig *params.ChainConfig, epochLength uint64) (*types.VoteAttestation, error) {
	...
	if header.Number.Uint64()%epochLength != 0 {
		...
		num := int(header.Extra[extraVanity])
		start := extraVanity + validatorNumberSize + num*validatorBytesLength + turnLengthSize
...
func (p *Stcons) getParent(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) (*types.Header, error) {
...
func (p *Stcons) verifyVoteAttestation(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Stcons) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Stcons) VerifyUnsealedHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	// check extra data
...
func (p *Stcons) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	err = p.verifyBlockTime(snap, header, parent)
	if err != nil {
		return err
	}
...
func (p *Stcons) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	for snap == nil {
		...
		// Unable to retrieve the exact EpochLength here; genesis and long reorgs
		// rebuild from a checkpoint using the current epoch length.
		offset := uint64(200)
		if number == 0 || (number%defaultEpochLength == offset && (len(headers) > int(params.FullImmutabilityThreshold))) {
			var (
				checkpoint    *types.Header
				blockHash     common.Hash
				blockInterval = blockIntervalMs(p.config)
			...
			if checkpoint != nil && blockHash != (common.Hash{}) {
				...
				// b. The function verifyBlockTime - This is also acceptable as it won't be invoked during `snap.apply`.
...
func (p *Stcons) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (p *Stcons) VerifyRequests(header *types.Header, Requests [][]byte) error {
	return nil
}
...
// from.
func (p *Stcons) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Stcons) prepareValidators(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (p *Stcons) prepareTurnLength(chain consensus.ChainHeaderReader, header *types.Header) error {
	epochLength, err := p.epochLength(chain, header, nil)
	if err != nil {
		return err
	}
...
func (p *Stcons) assembleVoteAttestation(chain consensus.ChainHeaderReader, header *types.Header) error {
	// === Step 1: Preconditions ===
	if header.Number.Uint64() < 3 || p.VotePool == nil {
		return nil
	}
...
func (p *Stcons) NextInTurnValidator(chain consensus.ChainHeaderReader, header *types.Header) (common.Address, error) {
...
func (p *Stcons) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (p *Stcons) prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	blockTime := p.nextBlockMilliTime(snap, header, parent)
...
// validators bytes (epoch only) + turnLength (epoch only) + 65 reserved seal bytes.
// Caller supplies the desired vanity in header.Extra; this function pads/truncates it.
func (p *Stcons) SetExtraData(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (p *Stcons) verifyValidators(chain consensus.ChainHeaderReader, header *types.Header) error {
	epochLength, err := p.epochLength(chain, header, nil)
	if err != nil {
		return err
	}
...
func (p *Stcons) verifyTurnLength(chain consensus.ChainHeaderReader, header *types.Header) error {
	epochLength, err := p.epochLength(chain, header, nil)
	if err != nil {
		return err
	}
...
func (p *Stcons) distributeFinalityReward(chain consensus.ChainHeaderReader, state vm.StateDB, header *types.Header,
...
func (p *Stcons) EstimateGasReservedForSystemTxs(chain consensus.ChainHeaderReader, header *types.Header) uint64 {
	parent := chain.GetHeaderByHash(header.ParentHash)
	if parent != nil && !isBreatheBlock(parent.Time, header.Time) {
...
// rewards given.
func (p *Stcons) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state vm.StateDB, txs *[]*types.Transaction,
	...
	cx := chainContext{ChainHeaderReader: chain, stcons: p}
...
func (p *Stcons) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB,
...
func (p *Stcons) IsActiveValidatorAt(chain consensus.ChainHeaderReader, header *types.Header, checkVoteKeyFn func(bLSPublicKey *types.BLSPublicKey) bool) bool {
	number := header.Number.Uint64()
	snap, err := p.snapshot(chain, number-1, header.ParentHash, nil)
	if err != nil {
...
func (p *Stcons) VerifyVote(chain consensus.ChainHeaderReader, vote *types.VoteEnvelope) error {
	...
	for addr, validator := range validators {
		if validator.VoteAddress == voteAddress {
			...
			metrics.GetOrRegisterCounter(fmt.Sprintf("stcons/VerifyVote/%s", addr.String()), nil).Inc(1)
			return nil
		}
	}
...
// with.
func (p *Stcons) Authorize(val common.Address, signFn SignerFn, signTxFn SignerTxFn) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *Stcons) IsLastBlockInTurn(chain consensus.ChainReader, header *types.Header) bool {
	snap, err := p.snapshot(chain, header.Number.Uint64()-1, header.ParentHash, nil)
	if err != nil {
		return false
	}
...
func (p *Stcons) Delay(chain consensus.ChainReader, header *types.Header, leftOver *time.Duration) *time.Duration {
	snap, err := p.snapshot(chain, header.Number.Uint64()-1, header.ParentHash, nil)
	if err != nil {
		return nil
	}

	delay := p.sealingDelay(header)
...
func (p *Stcons) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
	...
	go func() {
		...
		// Sign all the things!
		sig, err := signFn(accounts.Account{Address: val}, accounts.MimetypeStcons, StconsRLP(header, p.chainConfig.ChainID))
		if err != nil {
...
func (p *Stcons) shouldWaitForCurrentBlockProcess(chain consensus.ChainHeaderReader, header *types.Header, snap *Snapshot) bool {
	if header.Difficulty.Cmp(diffInTurn) == 0 {
		return false
	}
...
func (p *Stcons) EnoughDistance(chain consensus.ChainReader, header *types.Header) bool {
	snap, err := p.snapshot(chain, header.Number.Uint64()-1, header.ParentHash, nil)
	if err != nil {
		return true
	}
...
func (p *Stcons) IsLocalBlock(header *types.Header) bool {
	return p.val == header.Coinbase
}

func (p *Stcons) SignRecently(chain consensus.ChainReader, parent *types.Header) (bool, error) {
...
// current signer.
func (p *Stcons) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
	snap, err := p.snapshot(chain, parent.Number.Uint64(), parent.Hash(), nil)
	if err != nil {
		return nil
	}
...
func (p *Stcons) SealHash(header *types.Header) (hash common.Hash) {
...
func (p *Stcons) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "stcons",
		Version:   "1.0",
		Service:   &API{chain: chain, stcons: p},
		Public:    false,
	}}
}

// Close implements consensus.Engine. It's a noop for stcons as there are no background threads.
func (p *Stcons) Close() error {
	return nil
}
...
func (p *Stcons) getCurrentValidators(blockHash common.Hash, blockNum *big.Int) ([]common.Address, map[common.Address]*types.BLSPublicKey, error) {
	// block
...
func (p *Stcons) isIntentionalDelayMining(chain consensus.ChainHeaderReader, header *types.Header) (bool, error) {
...
func (p *Stcons) distributeIncoming(val common.Address, state vm.StateDB, header *types.Header, chain core.ChainContext,
...
func (p *Stcons) slash(spoiledVal common.Address, state vm.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mode systemTxMode, tracer *tracing.Hooks) error {
	// method
	method := "slash"
...
// get system message
func (p *Stcons) getSystemMessage(from, toAddress common.Address, data []byte, value *big.Int) *core.Message {
	return &core.Message{
		From:     from,
...
func (p *Stcons) applyTransaction(
	msg *core.Message,
	state vm.StateDB,
	header *types.Header,
	chainContext core.ChainContext,
...
func (p *Stcons) GetJustifiedNumberAndHash(chain consensus.ChainHeaderReader, headers []*types.Header) (uint64, common.Hash, error) {
...
func (p *Stcons) GetFinalizedHeader(chain consensus.ChainHeaderReader, header *types.Header) *types.Header {
...
func (p *Stcons) CheckFinalityAndNotify(chain consensus.ChainHeaderReader, targetBlockHash common.Hash, notifyFn func(finalizedHeader *types.Header)) {
...
// ===========================     utility function        ==========================
func (p *Stcons) backOffTime(snap *Snapshot, parent, header *types.Header, val common.Address) uint64 {
	if snap.inturn(val) {
...
func (p *Stcons) epochLength(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) (uint64, error) {
	if header == nil {
...
func (p *Stcons) BlockInterval(chain consensus.ChainHeaderReader, header *types.Header) (uint64, error) {
	if header == nil {
		return blockIntervalMs(p.config), errUnknownBlock
	}
	if header.Number.Uint64() == 0 {
		return blockIntervalMs(p.config), nil
	}
	snap, err := p.snapshot(chain, header.Number.Uint64()-1, header.ParentHash, nil)
	if err != nil {
		return blockIntervalMs(p.config), err
	}
	return snap.BlockInterval, nil
}

func (p *Stcons) NextProposalBlock(chain consensus.ChainHeaderReader, header *types.Header, proposer common.Address) (uint64, uint64, error) {
	snap, err := p.snapshot(chain, header.Number.Uint64(), header.Hash(), nil)
	if err != nil {
		return 0, 0, err
	}
...
func (p *Stcons) detectNewVersionWithFork(chain consensus.ChainHeaderReader, header *types.Header, state vm.StateDB) {
...
func (p *Stcons) GetAncestorGenerationDepth(_ *types.Header) uint64 {
