...
func VerifyEIP4844Header(config *params.ChainConfig, parent, header *types.Header) error {
	...
	// Non-eligible blocks must have no blob gas used.
...
func CalcExcessBlobGas(config *params.ChainConfig, parent *types.Header, headTimestamp uint64) uint64 {
	eip7918 := config.IsOsaka(config.LondonBlock, headTimestamp) && config.IsNotInSTC()
	bcfg, err := latestBlobConfig(config, headTimestamp)
	if err != nil {
		panic("calculating excess blob gas on nil blob config")
	}

	// For non-recalculation blocks (N % BlobEligibleBlockInterval != 1), inherit parent's ExcessBlobGas.
