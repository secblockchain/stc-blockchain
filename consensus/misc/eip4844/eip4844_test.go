...
// TestBlobEligibleBlockInterval tests blob transaction inclusion by block number.
// After Mendel fork, only blocks where N % BlobEligibleBlockInterval == 0 can include blob transactions.
func TestBlobEligibleBlockInterval(t *testing.T) {
	mendelTime := uint64(1000)
	cfg := *params.StconsTestChainConfig
